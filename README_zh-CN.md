...
> 当前仍是候选版，不是最终 benchmark。作者提供的最终裁定表将分层样本中的 1,400 组均记为“等价”，其中 42 组由原判“不等价”改判；这 42 组缺少逐组裁定理由。六处 DisasterM3 问题文本已修订，均属当前闭合答案主实验未覆盖的分割或报告任务。明确的下游许可、正式归档标识、完整生成与评分代码以及规则版本仍待补齐。
...
- 补记 42 组改判的逐组依据，公开适当的审查记录与主实验样本 ID／逐样本预测；若评估修订后的分割或报告任务，应重新运行对应模型。
