...
The six `rc2` text revisions are five segmentation records and one disaster-report record, all outside the reported five-task closed-answer baseline. They therefore do not alter question text in that main experiment. To make this boundary independently testable, publish the exact evaluation sample IDs and per-sample predictions. Run new predictions if the revised task types are evaluated in the future. Before claiming a reproducible final benchmark, also document the 42 changed decisions' per-pair reasons and record dataset/code revisions together. See `RELEASE_CHECKLIST.md`.
