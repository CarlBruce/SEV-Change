...
| Technical Validation | Machine structure/hash checks; author's stratified review records 1,400/1,400 final equivalent decisions after 42 re-adjudications | Record per-pair reasons for the 42 changed decisions; publish a suitable audit trail and exact main-experiment sample IDs/predictions; newly evaluate any revised task type before reporting its score |
...
| Citation and versioning | Candidate identifier `v0.1.0-rc2`; no final paper metadata | Mint/verify DOI, create an immutable release tag, add correct dataset/paper citation metadata, and synchronize all documentation |
